...
export function writeManifestFromDylib(dylibPath: string, targetFile: string = GEN_FILE): void {
    ...
    const nodeBindingPath = resolveNodeBinding();

    const relSpec = (p: string): string => {
        let spec = relative(dirname(targetFile), p).split(/[\\/]/).join("/");
    ...
    const manifest = [
        ...
        `import duckdbDylib from ${JSON.stringify(relSpec(dylibPath))} with { type: "file" };`,
        `import duckdbNodeBinding from ${JSON.stringify(relSpec(nodeBindingPath))} with { type: "file" };`,
        "",
        "export const DUCKDB_DYLIB: string | undefined = duckdbDylib;",
        "export const DUCKDB_NODE_BINDING: string | undefined = duckdbNodeBinding;",
        "",
    ].join("\n");
...
/**
 * The `duckdb.node` addon for the BUILD platform, resolved from node_modules
 * (#880). The platform packages are optionalDependencies of
 * `@duckdb/node-bindings`, so resolution starts from that package's own
 * directory - mirroring `packages/lib/src/duckdb/binding.ts`'s
 * `resolveSourceNodeBinding`, which this must stay in lockstep with.
 */
export function resolveNodeBinding(): string {
    // Resolve from packages/lib's dependency context - `@duckdb/node-bindings`
    // is @ax/lib's dep, not the repo root's, so a resolve from scripts/ misses.
    const require = createRequire(join(REPO_ROOT, "packages/lib/package.json"));
    const bindingsEntry = require.resolve("@duckdb/node-bindings");
    const fromBindings = createRequire(bindingsEntry);
    ...
    for (const candidate of [
        ...
        try {
            return fromBindings.resolve(candidate);
        } catch {
            /* try the next spelling */
        }
    }
    throw new Error(
        `no duckdb.node for ${key} in node_modules - is @duckdb/node-bindings installed for this platform?`,
    );
}
