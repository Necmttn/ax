...
published 6 wrapped cards - the dashboard landing serves them now`,
      ...
      {
        name: "directives",
        sub: ["mine", "list", "ngrams", "workflows"],
        job: "Mine candidate directive turns by lift, track proposals, inspect the n-gram lift table, and surface recurring skill-arc workflows.",
        signature: "ax directives mine [--days=N] [--emit-brief] | list [--status=...] | ngrams [--limit=N] | workflows [--emit-brief]",
        flags: [
          { flag: "--emit-brief", desc: "(mine/workflows) write agent brief to .ax/tasks/" },
...
12 candidates (90 days)  emit brief: ax directives mine --emit-brief`,
        detail: [
          ...
          "ax directives workflows mines recurring skill-arc workflows (ordered sequences of skills length 3–6 across ≥ 3 sessions, gapped + maximal; fixed 12-week window); --emit-brief writes .ax/tasks/workflows-<date>.md for agent review.",
    ...
    commands: [
      {
        ...
        signature: "ax hooks init | install <file>|--all | backtest <file> | cases | config",
        flags: [
...
enforce-worktree  -  14 days
  ...
  warn         0   defects fail OPEN`,
        detail: [
          ...
          "ax hooks install fans a hook file into provider configs idempotently; --all registers the single dispatcher (one spawn multiplexes every guard) and migrates off legacy per-guard entries; Codex requires interactive trust approval before new entries fire.",
    ...
    commands: [
      {
        ...
        signature: "ax profile show [--window=N] | publish [--yes] | widget [--yes] | unpublish | interview [submit]",
        flags: [
