    ...
    commands: [
      {
        ...
        flags: [
          ...
          { flag: "--all", desc: "(install) install every scaffolded guard at once" },
          { flag: "--dir=~/.ax/hooks", desc: "(install --all) workspace to scan" },
...
enforce-worktree  -  14 days
  ...
  warn         0   defects fail OPEN`,
        detail: [
          ...
          "ax hooks install fans a hook file into provider configs idempotently; --all installs every guard scaffolded in the workspace in one shot (no per-file dance); Codex requires interactive trust approval before new entries fire.",
