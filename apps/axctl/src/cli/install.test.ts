...
import {
    ...
    watcherProfileWidgetDoctorCheck,
    type DaemonStatus,
    type DoctorReport,
} from "./install.ts";
...
describe("cli install operations", () => {
    ...
    test("doctor flags watcher profile widget freshness drift", () => {
        const check = watcherProfileWidgetDoctorCheck(
            "<string>/bin/ax profile widget --if-stale=6 >>watcher.log 2&gt;&amp;1 || true</string>",
        );

        expect(check).toEqual({
            name: "watcher-profile-widget",
            ok: false,
            detail: "profile widget uses --if-stale=6; expected --if-stale=2; run 'axctl install' to refresh the watcher plist",
        });
    });

    test("doctor accepts the current watcher profile widget freshness gate", () => {
        const check = watcherProfileWidgetDoctorCheck(
            "<string>/bin/ax profile widget --if-stale=2 >>watcher.log 2&gt;&amp;1 || true</string>",
        );
