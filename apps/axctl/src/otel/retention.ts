...
export const retainRecentOtel = Effect.fn("otel.retention")(function* (write: CacheWriteService) {
    ...
    for (const table of OTEL_TABLES) {
        const rows = yield* write.rows(
            CountRow,
            `SELECT count(*) AS count FROM ${table} WHERE observed_at < CAST(CURRENT_TIMESTAMP AS TIMESTAMP) - INTERVAL '30 days'`,
        );
        deletedByTable[table] = rows[0]?.count ?? 0;
        yield* write.exec(
            `DELETE FROM ${table} WHERE observed_at < CAST(CURRENT_TIMESTAMP AS TIMESTAMP) - INTERVAL '30 days'`,
        );
    }
