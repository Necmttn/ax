...
describe("fetchSkillHygiene", () => {
    ...
    it("collapses plugin-namespace twins: sums invocations, keeps bare name", async () => {
        const rows = await run(
            ...
            makeMockDb([
                [
                    { sid: "skill:bare", invocations: 2, sessions: 2 },
                    { sid: "skill:ns", invocations: 3, sessions: 3 },
                ],
                [
        ...
        // merged 2+3=5 invocations, 2+3=5 sessions, >= 3 threshold, bare name kept
        expect(rows).toEqual([{ name: "foo", invocations: 5, sessions: 5 }]);
    });
