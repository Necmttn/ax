...
describe("sessions list", () => {
    dtest("lists only roots and enriches them from aggregate tables", async () => {
        const t = new Date();
        const fixture = await runWithPlatform(publishCacheFixture(tempDir("ax-sessions-list-"), dylibPath, (db) => Effect.gen(function* () {
        ...
        const result = await readThroughFixture(fixture, dylibPath, fetchSessionsList({ project: "ax" }));
        ...
        const children = await readThroughFixture(fixture, dylibPath, fetchSessionChildren("root"));
    ...
    dtest("binds filter values that contain quotes", async () => {
        const fixture = await runWithPlatform(publishCacheFixture(tempDir("ax-sessions-quote-"), dylibPath, (db) =>
            db.put("session", { id: "quoted", source: "claude", project: "user's repo", started_at: new Date() })));
        const result = await readThroughFixture(fixture, dylibPath, fetchSessionsList({ project: "user's repo" }));
