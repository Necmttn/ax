...
describe("fetchSessionInsights", () => {
    dtest("reads aggregate rows and maps cache dates to ISO strings", async () => {
        ...
        const fixture = await publishDashboardFixture(tempDir("ax-insights-"), dylibPath, (db) => Effect.gen(function* () {
        ...
        const payload = await runDashboardRead(fixture, fetchSessionInsights(SID));
    ...
    dtest("returns empty sections for an unknown session", async () => {
        const fixture = await publishDashboardFixture(tempDir("ax-insights-empty-"), dylibPath, () => Effect.void);
        const payload = await runDashboardRead(fixture, fetchSessionInsights(SID));
