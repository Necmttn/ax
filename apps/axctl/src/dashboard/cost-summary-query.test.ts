...
describe("cost summary", () => {
    dtest("reads bound rollups and pricing rows from DuckDB", async () => {
        const ts = new Date();
        const fixture = await runWithPlatform(publishCacheFixture(tempDir("ax-cost-summary-"), dylibPath, (db) => Effect.gen(function* () {
        ...
        const rollup = await readThroughFixture(fixture, dylibPath, fetchCostSummaryRollup({ source: "user's harness", sinceDays: 1, limit: 20 }));
        ...
        const pricing = await readThroughFixture(fixture, dylibPath, fetchPricingRows());
