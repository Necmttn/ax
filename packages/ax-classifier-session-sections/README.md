...
bun run classifiers:embedding-helper-graph-projection -- --review=.ax/experiments/embedding-helper-review-current.json --fixtures=packages/ax-classifier-session-sections/eval-fixtures/chunks.jsonl --out=.ax/experiments/embedding-helper-graph-projection-current.json --write-plan=.ax/experiments/embedding-helper-graph-write-plan-current.json
bun run classifiers:graph-write-plan-apply -- --write-plan=.ax/experiments/embedding-helper-graph-write-plan-current.json --out=.ax/experiments/embedding-helper-graph-apply-current.json --json
bun src/cli/index.ts classifiers package-operations --graph-health --graph-mode=embedding-helper --out=.ax/experiments/classifier-graph-health-embedding-helper-current.json
bun run classifiers:workflow-candidates -- --limit=20 --examples=10000 --out=.ax/experiments/workflow-candidate-report-helper-usefulness-transcript-current.json --json
bun run classifiers:workflow-candidates -- --source-kind=hybrid_window_classifier_projection --limit=20 --examples=10000 --out=.ax/experiments/workflow-candidate-report-helper-usefulness-hybrid-current.json --json
bun run classifiers:embedding-helper-graph-usefulness -- --graph-health=.ax/experiments/classifier-graph-health-embedding-helper-current.json --fixtures=packages/ax-classifier-session-sections/eval-fixtures/chunks.jsonl --workflow-report=.ax/experiments/workflow-candidate-report-helper-usefulness-transcript-current.json --workflow-report=.ax/experiments/workflow-candidate-report-helper-usefulness-hybrid-current.json --out=.ax/experiments/embedding-helper-graph-usefulness-current.json --json
