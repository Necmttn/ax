...
| Omp (oh-my-pi) transcripts | `omp` | `~/.omp/agent/sessions/**/*.jsonl` | `AX_OMP_DIR` | [`src/ingest/pi.ts`](../src/ingest/pi.ts) - omp is a Pi fork with an identical format, so the same parameterized parser (`OMP_PROVIDER` descriptor) runs; synthetic skills are `omp:<tool>` and sessions keep `omp` identity | `session`, `agent_provider`, `agent_session`, `agent_event`, `turn`, `tool`, `tool_call`, `invoked`, synthetic Omp tool skills |
