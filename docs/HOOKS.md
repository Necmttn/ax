...
The hook adds less than about 50 ms in the warm path. It reads the published
DuckDB snapshot. If the snapshot is unavailable, the hook still emits normal
stdout and writes an error to stderr.
...
Ax does not install a session-end hook. Such a hook fires for each turn and
blocks the agent while ingest runs.

Use the `/retro` skill to pull sessions that need review. The workflow records
**tried · worked · failed · next** and lets you review derived proposals. It
drains the pending review set on demand.
...
After a retro is saved, ax derives proposals from its clustered failures. A
proposal keeps a link to the retro that supplied the evidence. Use
`ax improve list` to review the resulting queue.
