...
## E217 - Embedding Helper Batch Dry-Run Sync

Question:

- Can a reviewer validate a filled embedding-helper batch without mutating the
  source review JSON?

Changes:

- Added `--dry-run` to `embedding_helper_review_batch.py --mode=sync`.
- Dry-run sync:
  - parses the batch Markdown
  - evaluates the would-be synced review with the same E213 status gate
  - writes a sync report
  - sets `dry_run: true` and `would_write_review: false`
  - does not write back to the source review JSON
- Added test coverage for dry-run sync report fields.

Command:

```sh
bun run classifiers:embedding-helper-review-batch -- --mode=sync --dry-run --review=.ax/experiments/embedding-helper-review-e210.json --batch=.ax/experiments/embedding-helper-review-batch-e216.md --out=.ax/experiments/embedding-helper-review-batch-dry-run-e217-report.json --json
```

Artifact:

- `.ax/experiments/embedding-helper-review-batch-dry-run-e217-report.json`

Results:

- Command exit status: `1`, expected because E216 is still unreviewed.
- Report schema: `ax.embedding_helper_review_batch_report.v1`
- Mode: `sync`
- `dry_run`: `true`
...
- E217 makes the reviewer workflow safer. A reviewer can now edit the focused
  Markdown batch, dry-run sync it to validate statuses/notes, then run real
  sync only when the report is clean enough to write.
- This still intentionally does not auto-review any candidate. The next useful
  step remains human review of the E216 batch.

## E218 - Embedding Helper Review Progress Report

Question:

- Can the embedding-helper reviewer see exactly what remains before export can
  unlock?

Changes:

- Extended `embedding_helper_review_batch.py --mode=evaluate` to report:
  - accepted/rejected/pending hard-negative counts
  - accepted/rejected/pending dedupe counts
  - selected item ids from the current batch Markdown
  - a concrete `next_action`
- Added test coverage for progress reporting and selected batch ids.

Command:

```sh
bun run classifiers:embedding-helper-review-batch -- --mode=evaluate --review=.ax/experiments/embedding-helper-review-e210.json --batch=.ax/experiments/embedding-helper-review-batch-e216.md --out=.ax/experiments/embedding-helper-review-progress-e218.json --json
```
...
- Command exit status: `1`, expected while review is pending.
- Report schema: `ax.embedding_helper_review_batch_report.v1`
- Mode: `evaluate`
- Decision: `needs_embedding_helper_review`
- Hard negatives accepted/rejected/pending: `0/0/15`
...
- E218 makes the review blocker explicit and machine-readable. The system can
  now show that the active batch covers 5 of 16 pending review items while the
  export gate remains closed.
- The next useful step remains reviewing E216, but the reviewer can now track
  progress without re-reading the full E210 artifact.

## E219 - Embedding Helper Partial Review Copy-Sync Smoke

Question:

- Can a partially reviewed embedding-helper batch be synced into a review copy
  and show reduced pending counts without mutating the canonical E210 review?

Changes:

- Added `--review-out` to `embedding_helper_review_batch.py --mode=sync`.
- When `--review-out` is set, sync writes the synced review JSON to that path.
- With `--dry-run --review-out`, canonical review JSON is not mutated, while a
  copy is still emitted for inspection.
- Sync reports now include:
  - `would_write_canonical_review`
  - `wrote_review_out`
  - `review_out` when a copy is written

Smoke setup:

- Created a synthetic partial-review batch from E216 with two edited hard
  negatives. This is explicitly not a promotion input and not a real human
  acceptance decision.

Command:

```sh
bun run classifiers:embedding-helper-review-batch -- --mode=sync --dry-run --review=.ax/experiments/embedding-helper-review-e210.json --review-out=.ax/experiments/embedding-helper-review-e219-synthetic-synced.json --batch=.ax/experiments/embedding-helper-review-batch-e219-synthetic.md --out=.ax/experiments/embedding-helper-review-partial-sync-e219-report.json --json
```

Artifacts:

- `.ax/experiments/embedding-helper-review-batch-e219-synthetic.md`
- `.ax/experiments/embedding-helper-review-e219-synthetic-synced.json`
- `.ax/experiments/embedding-helper-review-partial-sync-e219-report.json`

Results:

- Command exit status: `1`, expected because only two synthetic items were
  reviewed and the full review gate still has pending items.
- Canonical E210 review checksum before/after: unchanged.
- Report schema: `ax.embedding_helper_review_batch_report.v1`
- Mode: `sync`
- `dry_run`: `true`
...
- Decision: `needs_embedding_helper_review`
- Synthetic copy hard negatives accepted/rejected/pending: `1/1/13`
- Synthetic copy dedupe accepted/rejected/pending: `0/0/1`
- Failures still correctly report pending hard negatives and dedupe clusters.

Decision:

- E219 proves the review workflow can safely validate partial progress on a
  copy. That gives reviewers a safer path: edit batch, dry-run copy-sync,
...
## E221 - Embedding Helper Export Preview Package Operation

Question:

- Can the non-appendable embedding-helper preview export be discovered and run
  through classifier package operations without looking like a failed operation?

Changes:

- Added package operation `embedding-helper-export-preview`.
- Operation kind: `review`, not `publish`, because preview output is for
  inspection and remains non-appendable.
- Added `--preview-exit-zero` to `embedding_helper_export.py`.
- The preview operation uses both:
  - `--allow-partial-preview`
  - `--preview-exit-zero`
- The export report still carries the safety fields:
  - `decision: partial_embedding_helper_export_preview`
  - `appendable: false`
...
```sh
bun src/cli/index.ts classifiers package-operations --operation=embedding-helper-export-preview --preflight --out=.ax/experiments/classifier-package-operation-embedding-helper-export-preview-preflight-e221.json --json
bun src/cli/index.ts classifiers package-operations --operation=embedding-helper-export-preview --execute --out=.ax/experiments/classifier-package-execution-embedding-helper-export-preview-e221.json --json
```

Artifacts:

- `.ax/experiments/classifier-package-operation-embedding-helper-export-preview-preflight-e221.json`
- `.ax/experiments/classifier-package-execution-embedding-helper-export-preview-e221.json`
- `.ax/experiments/embedding-helper-fixture-preview-current.jsonl`
- `.ax/experiments/embedding-helper-dedupe-preview-current.json`
- `.ax/experiments/embedding-helper-export-preview-current-report.json`
...
- All declared outputs exist: `true`
- Preview fixture rows: `1`
- Underlying preview export report:
  - decision: `partial_embedding_helper_export_preview`
  - `appendable`: `false`
  - exported fixture rows: `1`
...
- E221 makes preview exports a first-class package review operation without
  weakening promotion safety. Package tooling can now run and track the preview
...
## E225 - Second Embedding Helper Reviewed Copy Smoke

Question:

- Can a second focused review batch shrink the embedding-helper hard-negative
  backlog further while keeping canonical E210 unchanged and export
  non-appendable?

Setup:

- Generated `.ax/experiments/embedding-helper-review-batch-e225.md` from
  `.ax/experiments/embedding-helper-review-e224-reviewed-copy.json`.
- The generated batch selected `5` hard negatives from the `10` still pending
  in the E224 reviewed copy:
  - `none-commit-request`
  - `none-next-step`
  - `none-task-recall`
  - `none-status-question`
  - `none-what-embedding`

Review proposal:

- Created `.ax/experiments/embedding-helper-review-batch-e225-reviewed.md`
  and marked all five selected hard negatives `accepted`.
- Rationale:
  - `none-commit-request`: git hygiene request before continuing; the contract
    treats git hygiene requests as `none` unless they correct a false commit
    claim.
  - `none-next-step`: asks for the next item after a completed step; this is
    conversational status/planning control, not approval or direction.
  - `none-task-recall`: asks what task was previously given; this is context
    recall.
  - `none-status-question`: asks for a brief work status update; this is status
    and control text.
  - `none-what-embedding`: asks for an explanation of embeddings and usage; it
    is explanatory context, not correction, verification, approval, recovery,
    or environment preference.

Commands:

```sh
bun run classifiers:embedding-helper-review-batch -- --review=.ax/experiments/embedding-helper-review-e224-reviewed-copy.json --fixtures=packages/ax-classifier-session-sections/eval-fixtures/chunks.jsonl --batch=.ax/experiments/embedding-helper-review-batch-e225.md --out=.ax/experiments/embedding-helper-review-batch-e225-report.json --limit=5 --json
bun run classifiers:embedding-helper-review-batch -- --mode=sync --dry-run --review=.ax/experiments/embedding-helper-review-e224-reviewed-copy.json --review-out=.ax/experiments/embedding-helper-review-e225-reviewed-copy.json --batch=.ax/experiments/embedding-helper-review-batch-e225-reviewed.md --out=.ax/experiments/embedding-helper-review-copy-sync-e225-report.json --json
bun run classifiers:embedding-helper-review-status -- --review=.ax/experiments/embedding-helper-review-e225-reviewed-copy.json --out=.ax/experiments/embedding-helper-review-status-e225-reviewed-copy.json --mode=evaluate --json
bun run classifiers:embedding-helper-export -- --allow-partial-preview --preview-exit-zero --review=.ax/experiments/embedding-helper-review-e225-reviewed-copy.json --status=.ax/experiments/embedding-helper-review-status-e225-reviewed-copy.json --fixtures=packages/ax-classifier-session-sections/eval-fixtures/chunks.jsonl --out=.ax/experiments/embedding-helper-fixture-preview-e225.jsonl --hints=.ax/experiments/embedding-helper-dedupe-preview-e225.json --report=.ax/experiments/embedding-helper-export-preview-e225-report.json --json
```

Artifacts:

- `.ax/experiments/embedding-helper-review-batch-e225.md`
- `.ax/experiments/embedding-helper-review-batch-e225-reviewed.md`
- `.ax/experiments/embedding-helper-review-batch-e225-report.json`
- `.ax/experiments/embedding-helper-review-e225-reviewed-copy.json`
- `.ax/experiments/embedding-helper-review-copy-sync-e225-report.json`
- `.ax/experiments/embedding-helper-review-status-e225-reviewed-copy.json`
- `.ax/experiments/embedding-helper-fixture-preview-e225.jsonl`
- `.ax/experiments/embedding-helper-dedupe-preview-e225.json`
- `.ax/experiments/embedding-helper-export-preview-e225-report.json`
...
- Batch generation:
  - decision: `embedding_helper_review_batch_ready`
  - selected hard negatives: `5`
  - pending hard negatives before sync: `10`
  - pending dedupe clusters before sync: `1`
- Copy-sync report:
  - mode: `sync`
  - `dry_run`: `true`
  - `would_write_review`: `false`
  ...
  - `wrote_review_out`: `true`
  - decision: `needs_embedding_helper_review`
  - hard negatives accepted/rejected/pending: `7/3/5`
  - dedupe accepted/rejected/pending: `0/0/1`
- Reviewed-copy status report:
  - decision: `needs_embedding_helper_review`
  - hard negatives accepted/rejected/pending: `7/3/5`
...
- Partial preview export:
  ...
  - `appendable`: `false`
  - accepted hard negatives: `7`
  - exported fixture rows: `7`
  - accepted dedupe clusters: `0`
  - exported dedupe hints: `0`
- Review checksums:
  - canonical E210: `5997295b73027794bae2c598a5207733eb75c8a9da95ae6f6fa2787c8423a541`
  - E224 reviewed copy: `7b9921bfe07b153cd22ad98666926fc2c4a20aa4b0fd40942cb18accea58c731`
  - E225 reviewed copy: `f41d3dc03ca8e9b2e982dd2a30709d4cd462d74c9164c0b9de5c64d965d152d3`

Decision:

- E225 continues the review-throughput path without weakening safety. The
  reviewed-copy backlog is now down to `5` hard negatives plus `1` dedupe
  cluster, while canonical E210 remains unchanged.
- The preview export now shows the likely accepted fixture append rows at `7`
  rows, but the report remains `appendable: false` because review is still
  incomplete.
- The next useful slice is the final hard-negative batch plus the dedupe
  cluster. After that, a human can decide whether to apply the reviewed-copy
  statuses back to canonical E210.

## E226 - Complete Embedding Helper Reviewed Copy Export Gate

Question:

- Can the final pending embedding-helper review items be judged in a copy and
  produce an appendable export report without mutating canonical E210?

Setup:

- Generated `.ax/experiments/embedding-helper-review-batch-e226.md` from
  `.ax/experiments/embedding-helper-review-e225-reviewed-copy.json` with
  `--limit=6`.
- The generated batch selected the remaining `5` hard negatives and the one
  pending dedupe cluster:
  - `none-results-comparison`
  - `none-maintenance-question`
  - `none-size-question`
...
- Created `.ax/experiments/embedding-helper-review-batch-e226-reviewed.md`.
- Marked all five selected hard negatives `accepted`:
  - result comparison and result summary are reporting requests
  - SurrealML maintenance is a current-information research request
  - classifier input size is explanatory model-context discussion
  - goal benchmark selection is planning/metric context
- Marked `embedding-dedupe-cluster/1` `accepted` because both fixtures are the
  same correction family: the agent claimed clean/committed state while
  `git status` still showed dirty classifier files.

Commands:

```sh
bun run classifiers:embedding-helper-review-batch -- --review=.ax/experiments/embedding-helper-review-e225-reviewed-copy.json --fixtures=packages/ax-classifier-session-sections/eval-fixtures/chunks.jsonl --batch=.ax/experiments/embedding-helper-review-batch-e226.md --out=.ax/experiments/embedding-helper-review-batch-e226-report.json --limit=6 --json
bun run classifiers:embedding-helper-review-batch -- --mode=sync --dry-run --review=.ax/experiments/embedding-helper-review-e225-reviewed-copy.json --review-out=.ax/experiments/embedding-helper-review-e226-reviewed-copy.json --batch=.ax/experiments/embedding-helper-review-batch-e226-reviewed.md --out=.ax/experiments/embedding-helper-review-copy-sync-e226-report.json --json
bun run classifiers:embedding-helper-review-status -- --review=.ax/experiments/embedding-helper-review-e226-reviewed-copy.json --out=.ax/experiments/embedding-helper-review-status-e226-reviewed-copy.json --mode=evaluate --json
bun run classifiers:embedding-helper-export -- --review=.ax/experiments/embedding-helper-review-e226-reviewed-copy.json --status=.ax/experiments/embedding-helper-review-status-e226-reviewed-copy.json --fixtures=packages/ax-classifier-session-sections/eval-fixtures/chunks.jsonl --out=.ax/experiments/embedding-helper-fixture-append-e226.jsonl --hints=.ax/experiments/embedding-helper-dedupe-hints-e226.json --report=.ax/experiments/embedding-helper-export-e226-report.json --json
```

Artifacts:

- `.ax/experiments/embedding-helper-review-batch-e226.md`
- `.ax/experiments/embedding-helper-review-batch-e226-reviewed.md`
- `.ax/experiments/embedding-helper-review-batch-e226-report.json`
- `.ax/experiments/embedding-helper-review-e226-reviewed-copy.json`
- `.ax/experiments/embedding-helper-review-copy-sync-e226-report.json`
- `.ax/experiments/embedding-helper-review-status-e226-reviewed-copy.json`
- `.ax/experiments/embedding-helper-fixture-append-e226.jsonl`
- `.ax/experiments/embedding-helper-dedupe-hints-e226.json`
- `.ax/experiments/embedding-helper-export-e226-report.json`
...
- Batch generation:
  - decision: `embedding_helper_review_batch_ready`
  - selected hard negatives: `5`
  - selected dedupe clusters: `1`
  - pending hard negatives before sync: `5`
  - pending dedupe clusters before sync: `1`
- Copy-sync report:
  - mode: `sync`
  - `dry_run`: `true`
  ...
  - hard negatives accepted/rejected/pending: `12/3/0`
  - dedupe accepted/rejected/pending: `1/0/0`
  - failures: `[]`
- Reviewed-copy status report:
  - decision: `ready_for_embedding_helper_export`
  - hard negatives accepted/rejected/pending: `12/3/0`
  ...
  - failures: `[]`
- Export report:
  - decision: `ready_to_append_embedding_helper_fixtures`
  - `partial_preview`: `false`
  - `appendable`: `true`
  - accepted hard negatives: `12`
  - exported fixture rows: `12`
  - accepted dedupe clusters: `1`
  - exported dedupe hints: `1`
  - label counts: `{ "none": 12 }`
  - failures: `[]`
- Review checksums:
  - canonical E210: `5997295b73027794bae2c598a5207733eb75c8a9da95ae6f6fa2787c8423a541`
  - E226 reviewed copy: `d610b0547abc437b330918743f7f178841edb7a10646b7924b499615270f0a4a`

Verification:

```sh
python3 -m json.tool .ax/experiments/embedding-helper-review-copy-sync-e226-report.json >/dev/null
python3 -m json.tool .ax/experiments/embedding-helper-review-status-e226-reviewed-copy.json >/dev/null
python3 -m json.tool .ax/experiments/embedding-helper-export-e226-report.json >/dev/null
python3 -m unittest packages/ax-classifier-session-sections/embedding_helper_review_batch_test.py packages/ax-classifier-session-sections/embedding_helper_review_status_test.py packages/ax-classifier-session-sections/embedding_helper_export_test.py
```

Passed: `16` targeted tests.

Decision:

- E226 completes the reviewed-copy path for the embedding-helper review queue.
  The copy is now export-ready and produces an appendable fixture/hint export.
- Canonical E210 remains unchanged, so production/canonical export is still not
  implicitly unlocked.
- The next useful slice is an explicit apply step: copy the reviewed statuses
  back to canonical E210 only if the reviewed-copy decisions are accepted, then
  run the canonical status/export path and append the accepted rows through the
  existing fixture append gate.

## E227 - Append Reviewed Embedding Helper Fixtures

Question:

- Can the completed embedding-helper review queue become an append-only
  experiment fixture set without mutating the canonical package fixtures?

Implementation:

- Applied the E226 reviewed copy back to local canonical review artifact
  `.ax/experiments/embedding-helper-review-e210.json`.
- Kept a local backup:
  `.ax/experiments/embedding-helper-review-e210-before-e227.json`.
- Extended `fixture_append.py` with an explicit
  `embedding-helper-hard-negative` source-group path:
  - accepted rows must keep label `none`
  - accepted rows must have review notes
  - accepted rows must preserve `source_candidate_id` and `source_fixture_id`
- Added package operation `embedding-helper-fixture-append`.

Commands:

```sh
bun run classifiers:embedding-helper-export -- --review=.ax/experiments/embedding-helper-review-e210.json --status=.ax/experiments/embedding-helper-review-status-e227-canonical.json --fixtures=packages/ax-classifier-session-sections/eval-fixtures/chunks.jsonl --out=.ax/experiments/embedding-helper-fixture-append-current.jsonl --hints=.ax/experiments/embedding-helper-dedupe-hints-current.json --report=.ax/experiments/embedding-helper-export-e227-report.json --json
bun src/cli/index.ts classifiers package-operations --operation=embedding-helper-fixture-append --execute --allow-expensive --out=.ax/experiments/classifier-package-execution-embedding-helper-fixture-append-e227.json
```

Artifacts:

- `.ax/experiments/embedding-helper-review-status-e227-canonical.json`
- `.ax/experiments/embedding-helper-export-e227-report.json`
- `.ax/experiments/embedding-helper-fixture-append-current.jsonl`
- `.ax/experiments/embedding-helper-dedupe-hints-current.json`
- `.ax/experiments/chunks-with-embedding-helper-fixtures-current.jsonl`
- `.ax/experiments/fixture-append-embedding-helper-current-report.json`
- `.ax/experiments/classifier-package-execution-embedding-helper-fixture-append-e227.json`

Results:

- Canonical review checksum before apply:
  `5997295b73027794bae2c598a5207733eb75c8a9da95ae6f6fa2787c8423a541`
- Canonical review checksum after apply:
  `d610b0547abc437b330918743f7f178841edb7a10646b7924b499615270f0a4a`
- Review status decision: `ready_for_embedding_helper_export`
- Hard negatives: `12` accepted, `3` rejected, `0` pending
- Dedupe clusters: `1` accepted, `0` pending
- Export decision: `ready_to_append_embedding_helper_fixtures`
- Export appendable: `true`
- Fixture append decision: `ready_to_write_combined_fixtures`
- Base rows: `124`
- Append rows: `12`
- Combined rows: `136`
- Append label counts: `{ "none": 12 }`

Decision:

- E227 closes the reviewed helper-to-fixture append path. The embedding/SVM
  helper is now valuable as a miner and reviewer assistant: it found reviewed
  hard negatives that can be appended without changing labels or weakening
  provenance.
- This still does not promote model output as facts. It only creates an
  experiment fixture set for downstream split and robustness gates.
...
Passed: `308` Python tests and `69` Bun tests. `bun run typecheck` exited `0`
with existing Effect advisories.

## E228 - Validate Embedding Helper Fixture Split And SetFit Robustness

Question:

- After appending the reviewed embedding-helper hard negatives, does the
  combined experiment fixture set remain split-safe and model-quality viable,
  or did we only add review volume?
...
bun run classifiers:split-audit -- --fixtures=.ax/experiments/chunks-with-embedding-helper-fixture-metadata-current.jsonl --group-field=pair_group --pair-field=pair_group --label-mode=coarse --seeds=7,13,42 --out=.ax/experiments/embedding-helper-fixture-split-audit-e228.json --json
bun run classifiers:setfit-robustness -- --fixtures=.ax/experiments/chunks-with-embedding-helper-fixture-metadata-current.jsonl --group-field=pair_group --label-mode=coarse --seeds=7,13,42 --epochs=1 --batch-size=8 --calibration-threshold=0.4 --out=.ax/experiments/setfit-robustness-embedding-helper-fixtures-e228.json --json
```

Artifacts:

- `.ax/experiments/chunks-with-embedding-helper-fixture-metadata-current.jsonl`
- `.ax/experiments/embedding-helper-fixture-split-audit-e228.json`
- `.ax/experiments/setfit-robustness-embedding-helper-fixtures-e228.json`
...
- Train/test rows per seed: `95` / `41`
- Overlap groups: `0`
...
- Runs: `3`
- Fixtures: `136`
- Train seconds total: `308.81`
- Macro F1 mean/min/max: `0.7858` / `0.7672` / `0.8189`
- Accuracy mean: `0.7968`
- None false-positive mean/max: `0.0513` / `0.0769`
- Calibrated summary at threshold `0.4` is effectively unchanged:
  - macro F1 mean/min/max: `0.7856` / `0.7672` / `0.8189`
  - none false-positive mean/max: `0.0513` / `0.0769`
- Per-seed highlights:
  - seed `7`: macro F1 `0.8189`, accuracy `0.8293`, none FP `0.0`
  - seed `13`: macro F1 `0.7713`, accuracy `0.7805`, none FP `0.0769`
  - seed `42`: macro F1 `0.7672`, accuracy `0.7805`, none FP `0.0769`

Decision:

- E228 is a real progress gate, not just fixture churn. The reviewed helper
  hard negatives increased the `none` fixture family and the grouped split
  remains leakage-free across all tested seeds.
- The SetFit gate now clears the working thresholds that earlier failed in
  E189: macro F1 is above `0.75` and `none` false-positive stays below `10%`
  on every tested seed.
- This supports the current hybrid architecture: embedding/SVM mines and
  explains review candidates; reviewed rows become append-only fixtures; SetFit
  is evaluated after the review and split gates.
- Canonical package fixture mutation should still be a separate reviewed
  promotion decision, because E228 only validates the experiment fixture file.

Verification:

```sh
python3 -m json.tool .ax/experiments/embedding-helper-fixture-split-audit-e228.json >/dev/null
python3 -m json.tool .ax/experiments/setfit-robustness-embedding-helper-fixtures-e228.json >/dev/null
```

## E229 - Embedding Helper Fixture Failure Analysis Gate

Question:

- After E228 passes the SetFit robustness gate, do residual confusion rows
  suggest promoting the reviewed helper fixture rows, mining another broad
  helper batch, or doing a narrower boundary review?

Implementation:

- Made `failure_analysis.py` gate-aware for robustness reports:
  - reports the source robustness decision and threshold checks
  - distinguishes failed gates from passed gates with residual review items
  - aggregates all-seed calibrated misses by source group
  - lists repeated misses across seeds, not only `none` false positives
- Added package operations for the embedding-helper augmented fixture path:
...
```sh
bun run classifiers:failure-analysis -- --robustness=.ax/experiments/setfit-robustness-embedding-helper-fixtures-e228.json --fixtures=.ax/experiments/chunks-with-embedding-helper-fixture-metadata-current.jsonl --out=.ax/experiments/setfit-failure-analysis-embedding-helper-fixtures-e229.json --json
cp .ax/experiments/setfit-robustness-embedding-helper-fixtures-e228.json .ax/experiments/setfit-robustness-embedding-helper-fixtures-current.json
bun src/cli/index.ts classifiers package-operations --operation=embedding-helper-fixture-failure-analysis --execute --out=.ax/experiments/classifier-package-execution-embedding-helper-fixture-failure-analysis-e229.json
```

Artifacts:

- `.ax/experiments/setfit-failure-analysis-embedding-helper-fixtures-e229.json`
- `.ax/experiments/setfit-failure-analysis-embedding-helper-fixtures-current.json`
- `.ax/experiments/classifier-package-execution-embedding-helper-fixture-failure-analysis-e229.json`

Results:

- Failure-analysis decision:
  `robust_with_residual_none_false_positive_review`
- Gate passed: `true`
- Gate observations:
  - source decision: `robust_enough`
  - source failures: `0`
  - macro F1 min: `0.7672` against threshold `0.75`
  - none false-positive max: `0.0769` against threshold `< 0.10`
- All-seed calibrated source-group miss counts:
  - `session-section-chunks`: `25`
- Embedding-helper appended-row miss count: `0`
- All-seed calibrated family counts:
  - `approval_boundary`: `5`
  - `label_boundary`: `14`
  - `missed_signal`: `4`
  - `none_false_positive`: `2`
- All-seed calibrated pair counts:
  - `correction_or_rejection_signal` ->
    `verification_or_recovery_signal`: `12`
  - `verification_or_recovery_signal` -> `approval`: `3`
  - `verification_or_recovery_signal` -> `none`: `3`
  - `approval` -> `verification_or_recovery_signal`: `2`
  - `none` -> `environment_or_preference_signal`: `2`
- Unique repeated none false-positive:
  - `session-section-chunks/none-model-size-question`
  - predicted as `environment_or_preference_signal`
  - seeds: `13`, `42`
  - max confidence: `0.4694`
- Repeated three-seed boundary misses include:
  - `verification-benchmark` missed as `none`
  - `correction-dirty-files-question`,
    `correction-not-committed`,
    `correction-failed-command-was-ignored`, and
    `correction-you-skipped-status-check` predicted as
    `verification_or_recovery_signal`
  - `recovery-updated-goal-doc` predicted as `approval`
  - `approval-keep-moving` predicted as either `none` or
    `verification_or_recovery_signal`
...
- E229 says not to run another broad embedding-helper mining batch yet. The
  reviewed helper append rows are not the residual miss source, and the gate
  is passing.
- Canonical promotion is still review-gated. Before promoting the appended
  helper rows into canonical fixtures, review the residual canonical boundary
  misses:
  - correction-vs-verification for workflow-state/git-hygiene corrections
  - benchmark/eval asks that should remain verification
  - model-size/download questions that should remain `none`
- This makes the next useful work a narrow fixture-boundary review or label
  contract refinement, not another expensive model run.
...
## E233 - Measure Promoted Helper Facts Against Workflow Ranking

Question:

- Do the persisted promoted helper facts produce a useful change in workflow or
  harness candidate ranking, or are they mostly review/debug evidence for now?

Implementation:

- Added `packages/ax-classifier-session-sections/embedding_helper_graph_usefulness.py`.
- Added `packages/ax-classifier-session-sections/embedding_helper_graph_usefulness_test.py`.
- Added `bun run classifiers:embedding-helper-graph-usefulness`.
- Added package operations:
  - `embedding-helper-usefulness-transcript-report`
  - `embedding-helper-usefulness-hybrid-report`
  - `embedding-helper-graph-usefulness`
- The usefulness report reads persisted embedding-helper graph health, joins
  promoted helper facts back to canonical fixture text, scans workflow candidate
  examples, and reports candidate examples that overlap promoted `none`
  controls. It emits an estimated adjusted support count, but does not change
  ranking behavior.

Commands:

```sh
bun src/cli/index.ts classifiers package-operations --operation=embedding-helper-usefulness-transcript-report --execute --out=.ax/experiments/classifier-package-execution-embedding-helper-usefulness-transcript-report-e233.json
bun src/cli/index.ts classifiers package-operations --operation=embedding-helper-usefulness-hybrid-report --execute --out=.ax/experiments/classifier-package-execution-embedding-helper-usefulness-hybrid-report-e233.json
bun src/cli/index.ts classifiers package-operations --operation=embedding-helper-graph-usefulness --execute --out=.ax/experiments/classifier-package-execution-embedding-helper-graph-usefulness-e233.json
```
...
- `.ax/experiments/embedding-helper-graph-usefulness-expanded-e233.json`
- `.ax/experiments/classifier-package-execution-embedding-helper-usefulness-transcript-report-e233.json`
- `.ax/experiments/classifier-package-execution-embedding-helper-usefulness-hybrid-report-e233.json`
- `.ax/experiments/classifier-package-execution-embedding-helper-graph-usefulness-e233.json`

Results:

- Promoted helper facts considered: `12`
- Workflow reports scanned: `2`
- Expanded candidate groups:
  - transcript: `15`
  - hybrid: `3`
- Matched candidate groups: `1`
- Matched candidate examples: `1`
- Estimated suppressed support:
  - transcript: `0 / 250`
  - hybrid: `1 / 92`
- The single match:
  - candidate group: `classifier_candidate_group:hybrid-window/environment_or_preference_signal`
  - action: `record_guidance_or_environment_preference`
  - support before/after helper adjustment: `50 / 49`
  - promoted helper control: `session-section-chunks/none-maintenance-question`
  - match score: `0.7333`
  - example text: user asks when SurrealML was last worked on and whether it is
    actively maintained.

Decision:

- The promoted helper facts are queryable and do catch one plausible noisy
  hybrid candidate, but the effect is too small to justify changing workflow
  ranking weights or adding a hard ranking gate today.
- Use helper facts as review/debug explanations for now. The next useful work
  is either to collect more promoted controls from broader review queues, or to
  expand the usefulness scan from candidate examples to all candidate evidence
  rows before making helper facts affect ranking.
...
## E237 - Add Helper Review Hints To Candidate Blocks

Question:

- Once a promoted helper control is attached to a workflow topic pack, can the
  reviewer see the practical decision implication without scrolling to a
  separate explanation section?

Implementation:

- Grouped topic helper explanations by candidate while rendering evidence pack
  markdown.
- When a candidate is matched by promoted helper controls whose reviewed label
  is `none`, the adjacent candidate block now renders:
  - `Helper review hint: review-as-noise`
  - matched helper control count
  - rationale naming the promoted `none` control
  - suggested reviewer verdict `reject`
- This remains a review hint only. It does not alter ranking, support counts,
  promotion gates, or persisted facts.

Commands:

```sh
bun src/cli/index.ts classifiers workflow-candidates --topic-report --search=surrealml --source-kind=hybrid_window_classifier_projection --limit=10 --examples=100 --include-helper-facts --out=.ax/experiments/workflow-topic-report-helper-review-hints-e237.json --evidence-pack=.ax/experiments/workflow-topic-evidence-pack-helper-review-hints-e237.md --json
```

Artifacts:

- `.ax/experiments/workflow-topic-report-helper-review-hints-e237.json`
- `.ax/experiments/workflow-topic-evidence-pack-helper-review-hints-e237.md`

Results:

- The adjacent candidate block for
  `classifier_candidate_group:hybrid-window/environment_or_preference_signal`
  now includes:
  - `Helper review hint: review-as-noise`
  - `Helper matched controls: 1`
  - helper rationale pointing to
    `session-section-chunks/none-maintenance-question`
  - `Suggested reviewer verdict: reject`
- The detailed `Promoted Helper Controls` section remains present with the
  promoted fixture and five nearest reviewed fixtures.

Decision:

- This is the first reviewer-actionable output from the helper layer. It makes
  the E234 weak usefulness signal usable in the human review workflow without
  pretending it is strong enough for automatic suppression.
- Next useful work is persisting reviewer outcomes from these hinted packs, or
  collecting broader helper controls.
