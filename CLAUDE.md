...
`ax cost cache [--days=N] [--limit=N] [--json]` - cache-bust cost attribution (#868). Reads the `cache_bust_event` ledger, derived by the **cache-bust SQL model** (`apps/axctl/src/ingest/models/cache-bust-event.sql`, stage key `cache-bust`, rides the #888 runner): one row per `turn_token_usage` row carrying a `cache_miss_reason_type` (id == ttu.id, incremental by the since-window, version-marked cutover). Each bust carries TWO independent prices - `bust_cost_usd` (the ingest pricer's `estimated_cache_creation_cost_usd`) and `corroborated_cost_usd` (flat-rate `tokens × agent_model.cache_creation_per_million_usd`, deliberately no 200k tier / fast multiplier) - so the planned proposal-minting corroboration guard (±25%) never compares the pricer with itself. Output: busts by cause, top offenders by native skill/agent attribution, claude-only coverage line, corroboration verdict, and a trimming ~$N/week estimate. Query: `apps/axctl/src/queries/cache-bust.ts`. MCP: `cost_cache`.

**Slice B: proposal minting (#868).** The `proposals` ingest stage
(`apps/axctl/src/ingest/derive-proposals.ts`) auto-mints top cache-busting
offenders (native `attribution_skill`/`attribution_agent` from
`cache_bust_event`) as guidance-form proposals, `section='cache-lens'` (same
discriminator pattern as directives/workflows) - triaged via `ax improve list`
...
(`apps/axctl/src/queries/cache-bust.ts`), the minting-side sibling of
`fetchCacheBustCost`; the stage runs `cache_bust_event`-dependent (declared dep
`"cache-bust"`) since candidates read THIS run's live writes, not the published
snapshot.
